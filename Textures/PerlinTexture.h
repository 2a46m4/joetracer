...
public:
	PerlinTexture(int sc)
	{
		...
		{
			*(randomVec + i) = unitVec(Vec(randomNum(-1.0f, 1.0f), randomNum(-1.0f, 1.0f), randomNum(-1.0f, 1.0f)));
		}
		...
		for (int di = 0; di < 2; di++)
		{
			for (int dj = 0; dj < 2; dj++)
			{
				for (int dk = 0; dk < 2; dk++)
				{
					...
					c[di][dj][dk] = randomVec[permX[(i + di) & 255] ^ permY[(j + dj) & 255] ^ permZ[(k + dk) & 255]];
				}
			}
		}
...
private:
	...
	// Given coordinates u, v, w, and gradient vector grid c, returns the interpolated result
	static double perlinInterpolation(Vec c[2][2][2], double u, double v, double w)
	{
		...
		for (int i = 0; i < 2; i++)
		{
			for (int j = 0; j < 2; j++)
			{
				for (int k = 0; k < 2; k++)
				{
					// this is the vector away from the lattice point.. gets bigger as it gets farther away from the point
	...
	static double trilinearInterpolation(double c[2][2][2], double u, double v, double w)
	{
		...
		v = v * v * (3 - 2 * v);
		w = w * w * (3 - 2 * w);
		// I think this is the interpolation step, which basically interpolates three dimensions according to their nearest grid
		for (int i = 0; i < 2; i++)
		{
			for (int j = 0; j < 2; j++)
			{

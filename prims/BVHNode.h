    ...
    class BVHNode : public Hittable
    {
        ...
        BVHNode(const std::vector<Hittable *> &objects, size_t start, size_t end, double t0, double t1)
        {
            ...
            auto boxCompare = [axis](Hittable *a, Hittable *b)
            {
                aabb boxA;
                aabb boxB;
                if (!a->boundingBox(0, 0, boxA) || !b->boundingBox(0, 0, boxB)) printf("Bounding box not possible for an object\n");
                if (axis == 0) // X
                {
            ...
            else
            {
                ...
                right = new BVHNode(objs, mid + 1, end, t0, t1);
            }

            aabb leftBox, rightBox;

            if(!left->boundingBox(t0, t1, leftBox) || !right->boundingBox(t0, t1, rightBox)) printf("Bounding box is not possible for an object\n");

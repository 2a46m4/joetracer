    ...
    while (!quit)
    {
...

        {
            ...
            if (ImGui::BeginTabBar("Settings", tab_bar_flags))
            {
                ...
                {
                    ImGui::DragInt("Samples", &s->samples, 0.5f, 0, 1000, "%d", NULL);
                    ImGui::DragInt("Bounces", &s->bounces, 0.5f, 0, 20, "%d", NULL);
                    ImGui::EndTabItem();
                }
        ...
        //     ImGui::Text("Render Settings");                    // Display some text (you can use a format strings too)
